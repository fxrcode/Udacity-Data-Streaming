...
customerEncodedEntriesStreamingDF = spark.sql("select zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
customerDecodedEntriesStreamingDF = customerEncodedEntriesStreamingDF \
        .withColumn("customer", unbase64(customerEncodedEntriesStreamingDF.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
customerDecodedEntriesStreamingDF.withColumn("customer", from_json("customer", redisCustomerSchema)) \
        .select(col("customer.*")) \
...
emailRiskStreamingDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("""
    email = customer
"""))

# emailRiskStreamingDF.writeStream.outputMode("append").format("console").option("trucate", "false").start().awaitTermination()
...
emailRiskStreamingDF.selectExpr("cast(customer as string) key", "to_json(struct(*)) value") \
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
